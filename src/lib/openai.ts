// OpenAI API клиент для генерации тестов
import { OpenAI } from 'openai';

// Инициализация OpenAI клиента
const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY, // Получаем ключ из переменных окружения
  dangerouslyAllowBrowser: true, // В production версии установить в false
});
  ...
  try {
    ...
    try {
      const response = await openai.chat.completions.create({
        model: "gpt-3.5-turbo", // Можно использовать gpt-4 для более качественных результатов
        messages: [
      ...
      // Парсим JSON
      const questions: TestQuestion[] = JSON.parse(content);
      return questions;
    } catch (apiError) {
      console.error("Ошибка API OpenAI, использую локальную генерацию:", apiError);
...
export async function analyzeText(text: string): Promise<string[]> {
  try {
    try {
      ...
      console.error("Ошибка API OpenAI при анализе текста, использую локальную генерацию:", apiError);
